...
assert.equal(engine.defaults.allAtOnceMinZoom, 100);
assert.equal(engine.defaults.payoffKeyMode, "overlay");
const fixedScaleConfig = { ...engine.defaults, profile: engine.profileValues(profile) };
assert.deepEqual(engine.validateConfig(fixedScaleConfig), []);
embeddedData.cs_all_at_once_default_zoom = "75";
embeddedData.cs_all_at_once_min_zoom = "75";
embeddedData.cs_inactivity_seconds = "120";
const normalizedLegacyScaleConfig = engine.readConfig(bank);
assert.equal(normalizedLegacyScaleConfig.allAtOnceDefaultZoom, 100);
assert.equal(normalizedLegacyScaleConfig.allAtOnceMinZoom, 100);
assert.equal(normalizedLegacyScaleConfig.payoffKeyMode, "overlay");
assert.deepEqual(engine.validateConfig(normalizedLegacyScaleConfig), []);
embeddedData.cs_payoff_key_mode = "below";
const belowKeyConfig = engine.readConfig(bank);
assert.equal(belowKeyConfig.payoffKeyMode, "below");
assert.deepEqual(engine.validateConfig(belowKeyConfig), []);
embeddedData.cs_payoff_key_mode = "overlay";
...
assert.deepEqual(engine.cardFooterLines(profile, progressState, round("trio_a"), round("trio_a").cards[0]), ["3 cards left", "for +30 pts."]);
assert.equal(engine.cardFooterText(profile, progressState, round("trio_a"), round("trio_a").cards[0]), "3 cards left for +30 pts.");
assert.deepEqual(engine.cardFooterLines(profile, progressState, round("infinite_scroll", null, 7), round("infinite_scroll", null, 7).cards[0]), ["4 cards left", "in this run"]);
...
assert.equal(engine.cardDisplayText(profile, progressState, round("trio_a"), round("trio_a").cards[0], defaultCardTextConfig).footer, "1 card left for +30 pts.");
assert.deepEqual(engine.cardDisplayText(profile, progressState, round("trio_a"), round("trio_a").cards[0], defaultCardTextConfig).footerLines, ["1 card left", "for +30 pts."]);
...
assert.equal(engine.cardDisplayText(profile, progressState, round("trio_a"), round("trio_a").cards[0], hiddenNonSimplePayoffsConfig).footer, "1 card left for +30 pts.");
